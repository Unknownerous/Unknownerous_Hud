...
local playerAge = "Inconnu"

-- Dès que le joueur spawn
AddEventHandler('playerSpawned', function()
    ESX.TriggerServerCallback('player:fetchInformations', function(data)
        if data then
            playerName = data.name or "Inconnu"
            playerAge = data.age or "Inconnu"
            
            SendNUIMessage({
                status = 'identity',
                data = { name = playerName, age = playerAge }
            })
        end
    end)
end)